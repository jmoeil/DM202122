...
Il existe de nombreuses méthodes pour dériver ce résultat. Ici, nous allons partir d'un développement extrêmement général, lequel nous permettra d'obtenir une équations différentielle dont la solution sera la fonction d'onde recherchée. A partir de là, nous pourrons introduire les opérateurs d'échelle en \ref{opérateur d'échelle}, ainsi que l'opérateur déplacement en \ref{opérateur déplacement}.

\paragraph{Développement général}
Introduisons, de manière extrêmement générale, les obserables $\bm{A}$ et $\bm{B}$. Soit $\ket{\psi}$ dans notre espace de Hilbert. Imposons la notation

\begin{equation*}
  \begin{cases}
    \sigma_A\ket{\psi} = \left(\bm{A}-\expval{\bm{A}}\right)\ket{\psi} = \ket{f}\\
    \sigma_B\ket{\psi} = \left(\bm{B}-\expval{\bm{B}}\right)\ket{\psi} = \ket{g}
  \end{cases}
\end{equation*}

Nous avons donc $\sigma_A = \bm{A}-\expval{\bm{A}}$ et $\sigma_B = \bm{B}-\expval{\bm{B}}$. L'\href{https://en.wikipedia.org/wiki/Cauchy%E2%80%93Schwarz_inequality}{\emph{inégalité de Cauchy-Schwarz}} nous indique que
\begin{equation}
  \label{CS}
  \braket{f}\braket{g} \geq \norm{\braket{f}{g}}^2.
\end{equation}
L'égalité est stricte si et seulement si $\ket{f}$ et $\ket{g}$ sont colinéaires, c'est à dire si et seulement si $\ket{f} = \lambda\ket{g}$, où $\lambda$ est un nombre \emph{à priori} dans le plan complexe. Réalisons de suite que cette égalité implique que $\ket{\psi}$ est un état propre de l'opérateur $\bm{A}-\lambda \bm{B}$ ; de valeur propre $\expval{\bm{A}}-\lambda\expval{\bm{B}}$. En effet,
\begin{align}
  \left(\bm{A}-\expval{\bm{A}}\right)\ket{\psi} &= \lambda\left(\bm{B}-\expval{\bm{B}}\right)\ket{\psi}\notag\\
  \left(\bm{A}-\lambda \bm{B}\right)\ket{\psi} &= \left(\expval{\bm{A}}-\lambda\expval{\bm{B}}\right)\ket{\psi}\label{EVP 1}
\end{align}
Développons à présent le terme $\braket{f}{g}$ dans l'inégalité \eqref{CS}.
\begin{equation}
  \braket{f}{g} = \bra{\psi}\sigma_A\sigma_B\ket{\psi} = \frac{1}{2}\bra{\psi}\sigma_A\sigma_B-\sigma_B\sigma_A\ket{\psi}+\frac{1}{2}\bra{\psi}\sigma_B\sigma_A+\sigma_A\sigma_B\ket{\psi}  
\end{equation}
Le premier terme de la somme traduit l'inégalité de Heisenberg $\sigma_A\sigma_B \geq \frac{1}{2}\expval{[\bm{A},\bm{B}]}$. Pour obtenir une égalité stricte, nous devons donc imposer que le second terme de la somme soit nulle. En particulier, on se retrouve alors à devoir imposer les deux conditions suivantes:
\begin{subequations}
  \begin{align}
    \bra{\psi}\sigma_B\sigma_A+\sigma_A\sigma_B\ket{\psi} &= 0\label{1}\\
    \ket{f} &= \lambda\ket{g}\label{2}
  \end{align}
\end{subequations}
En particulier, en injectant \eqref{2} dans \eqref{1}, nous trouvons que
\begin{equation}
  \left(\lambda+\lambda^*\right)\expval{g} = 0
\end{equation}
ce qui implique que $\lambda$ est un nombre 'complexe pure', puisque $\expval{g}$ doit être réel. On peut alors le réécrire sous la forme $\lambda=i\mu$, pour tout $\mu$ sur la droite réelle. Nous pouvons directement réécrire l'équation aux valeurs propres \eqref{EVP 1} sous la forme
\begin{equation}
  \left(A-i\mu B\right)\ket{\psi} = \left(\expval{A}-i\mu\expval{B}\right)\ket{\psi}\label{EVP 2}
\end{equation}
De plus, nous avons aussi que $\sigma_A^2 = \mu^2\sigma_B^2 \Rightarrow \mu = \pm\frac{\sigma_A}{\sigma_B}$.

\paragraph{Cas des opérateurs position et impulsion}
Appliquons à présent ces résultats aux opérateurs position et impulsion ; en posant $\bm{A} = \bm{X}$ et $\bm{B} = \bm{P}$.

En imposant l'égalité \eqref{EVP 2} pour les opérateurs de ce pagaraphe, nous nous retrouvons alors
\begin{equation}
  \left(x-i\mu p\right)\psi(x) = \left(\expval{\bm{X}}-i\mu\expval{\bm{P}}\right)\psi(x)\label{EVP XP}
\end{equation}
En particulier, notons que dans la représentation $\bm{X}$, l'opérateur impulsion $\bm{P} = -i\h\frac{\partial}{\partial x}$. Nous obtenons dès lors une \emph{équation différentielle}, que nous pouvons résoudre par séparation des variables.
\begin{align}
  \left(x-\mu\h\frac{\partial}{\partial x}\right)\psi(x) &= \left(\expval{\bm{X}}-i\mu\expval{\bm{P}}\right)\psi(x)\notag\\
  \frac{\partial \psi}{\partial x} &= \frac{1}{\mu\h}\left(x-\expval{\bm{X}}+i\mu\expval{\bm{P}}\right)\psi(x)\notag
\end{align}
La solution de cette équation différentielle est de la forme
\begin{align}
  \psi(x) &= \psi_0 \exp(\frac{x^2-2x\left(\expval{\bm{X}}-i\mu\bm{P}\right)}{2\mu\h})\notag
\end{align}
où $\psi_0$ est une constante de normalisation, que nous déterminerons au paragraphe suivant. En réarangeant les termes dans l'exponentielle, nous avons 
\begin{align}
  \psi(x) &= \psi_0\exp(-\frac{-x^2+2x\expval{\bm{X}}}{2\mu\h}+\frac{i\expval{\bm{P}}}{\h})\notag\\
  &= \psi_0\exp(-\frac{\left(x-\expval{\bm{X}}^2\right)\color{red}+\expval{\bm{X}}^2\color{black}}{2\mu\h}+\frac{i\expval{\bm{P}}x}{\h})
\end{align}
ce qui est la forme recherchée. La fonction d'onde décrivant un état cohérent est donc donnée par une gaussienne.
\paragraph{Constante de normalisation}
Il ne nous reste plus qu'à déterminer la valeur de $\psi_0\in\mathbb{R}$. Pour ce faire, nous exploitons la propriété suivantes
...
\begin{align}
  \bra{\psi}\bm{a}\ket{\psi} &= \alpha\braket{\psi} = \frac{1}{\sqrt{2}}\left(\frac{\expval{\bm{X}}}{l}+\frac{il\expval{\bm{P}}}{\h}\right).
\end{align}

Dès lors, le problème se réduit à résoudre
\begin{align}
  \frac{1}{\sqrt{2}}\left(\frac{\bm{X}}{l}+\frac{il\bm{P}}{\h}\right)\ket{\psi} &= \frac{1}{\sqrt{2}}\left(\frac{\expval{X}}{l}+\frac{il\expval{P}}{\h}\right)\\
  \left(\bm{X}-\expval{\bm{X}}\right)\ket{\psi} &= -\frac{il^2}{\h}\left(\bm{P}-\expval{\bm{P}}\right)\ket{\psi}
\end{align}
qui est l'équation : nous l'avons déjà résolue! Nous voyons en particulier que, à $l$ fixé, la solution est unique. Nous pouvons donc écrire $\ket{\psi} = \ket{\alpha}$. La fonction d'onde $\braket{x}{\psi}=\braket{x}{\alpha}$ est déterminée par la gaussienne que nous avons trouvé en \ref{fonction d'onde d'un état cohérent}.

\subsubsection{Intégrale gaussienne}

Nous voulons déterminer la valeur du produit scalaire $\braket{0}{\alpha}$. Pour ce faire, insérons la relation de complétude $\int_{-\infty}^{+\infty} dx\; \ket{x}\bra{x} = \mathbb{I}$:
\begin{align}
  \braket{0}{\alpha} &= \int_{-\infty}^{+\infty}dx\; \braket{0}{x}\braket{x}{\alpha}
\end{align}

Notons que le terme $\braket{0}{\alpha}$ est l'état fondamental de l'oscillateur  harmonique, que nous avons calculé dans le cours théorique. Le terme $\braket{x}{\alpha}$ est la fonction d'onde d'un état cohérent, déterminé en \ref{fonction d'onde d'un état cohérent}. Dès lors, nous voulons résoudre l'\emph{intégrale gaussienne}
\begin{align}
  \braket{0}{\alpha} &= \frac{\exp(-\frac{i}{2\h}\expval{\bm{X}}\expval{\bm{P}}-\frac{\expval{\bm{X}}^2}{2l^2})}{\sqrt{\pi}l}\int_{-\infty}^{+\infty}dx \; e^{-\frac{x^2}{l^2}+\frac{\sqrt{2}\alpha}{l}x}\notag\\
  &= \frac{\exp(-\frac{i}{2\h}\expval{\bm{X}}\expval{\bm{P}}-\frac{\expval{\bm{X}}^2}{2l^2})}{\sqrt{\pi}l} \sqrt{\pi}le^{\frac{\norm{\alpha}^2}{2}}\\
  &=  \exp(-\frac{i}{2\h}\expval{\bm{X}}\expval{\bm{P}}-\frac{\expval{\bm{X}}^2}{2l^2}+\frac{\norm{\alpha}^2}{2}) = \color{red} e^{-\frac{1}{2}\norm{\alpha}^2} \color{black}
\end{align}
...
Dans cette partie, nous voulons étudier un modèle très simple de la décohérence quantique. Pour ce faire, nous allons exploiter l'interprétation en terme de particules des opérateurs de déplacement étudiés en \ref{opérateur déplacement}.
