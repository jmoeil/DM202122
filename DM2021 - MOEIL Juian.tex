...
\begin{equation*}
  \begin{cases}
    \sigma_A = \left(\bm{A}-\expval{\bm{A}}\right)\ket{\psi} = \ket{f}\\
    \sigma_B = \left(\bm{B}-\expval{\bm{B}}\right)\ket{\psi} = \ket{g}
  \end{cases}
\end{equation*}

De même, prenons $f = \bm{A}-\expval{\bm{A}}$ et $g = \bm{B}-\expval{\bm{B}}$. L'\href{https://en.wikipedia.org/wiki/Cauchy%E2%80%93Schwarz_inequality}{\emph{inégalité de Cauchy-Schwarz}} nous indique que
\begin{equation}
  \label{CS}
...
\begin{equation}
  \braket{f}{g} = \bra{\psi}fg\ket{\psi} = \frac{1}{2}\bra{\psi}fg-gf\ket{\psi}+\frac{1}{2}\bra{\psi}gf+fg\ket{\psi}  
\end{equation}
...
\begin{subequations}
  \begin{align}
    \bra{\psi}gf+gf\ket{\psi} &= 0\label{1}\\
...
Nous obtenons alors que $\psi(x) = \psi_0 \; e^{-\frac{\left(x-\expval{X}\right)^2}{2l^2}+\frac{i\expval{P}x}{\hbar}}$, ce qui conclut presque notre discussion. Il nous reste plus qu'à déterminer la constante de normalisation $\psi_0\in\mathbb{R}$. Pour ce faire, exploitons la propriété suivante:
