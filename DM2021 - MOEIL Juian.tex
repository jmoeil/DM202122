...
Considérons l'opérateur déplacement de paramètre $\alpha+\beta$. Celui-ci vaut - par définition,
\begin{align}
...
\end{align}
\begin{remark}
  Le facteur de phase $e^{iIm(\alpha^*\beta)}$ est une phase globale, et n'a donc aucune importance physique.
\end{remark}
...
\begin{align}
  ...
  \expval{P}(t) &= -\sqrt{2m\h\omega}\norm{\alpha}\sin(\omega t-\theta) \label{mean P}
\end{align} 
...
\begin{itemize}
  ...
  \item La moyenne de la position \eqref{mean position} et la moyenne de l'impulsion \eqref{mean P} oscillent \emph{de la même manière que dans la limite classique} ! On voit donc ici la véritable puissante de la notion d'état cohérent.
\end{itemize} 
