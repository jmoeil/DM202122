...
On se restreint au cas unidimensionnel, en se munissant des opérateurs position $\bm{X}$ et impulsion $\bm{P}$, satisfaisant la relation de commutation canonique $[\bm{X},\bm{P}] = i\hbar = -\frac{\h}{i}$.

\subsection*{Introduction des concepts clés}
\begin{definition}[Ecart quadratique moyen]
  Soit $\bm{A}$ une observable. L'écart quadratique moyen de $\bm{A}$ dans l'état $\rho = \ketbra{\psi}$ pour tout $\psi\in$ H est 
  \begin{equation}
...
Soit $\bm{a} = \frac{1}{\sqrt{2}} \left(\frac{\bm{X}}{l}+\frac{il\bm{P}}{\hbar}\right)$ - l'opérateur destruction. Nous comprendrons plus tard, en étudiant ses propriétés, la logique derrière son nom. De sa définition, nous trouvons directement l'expression de la commutation des bosons:
\begin{equation}
...
\begin{remark}
  Dans la suite, nous négligerons souvent d'indiquer l'identité $\mathbb{I}$; pour priviligier de ne 'rien mettre'. Cependant, de temps en temps, nous insisterons que quelque chose est multiplié par $\mathbb{I}$ en l'indiquant explicitement (surtout dans la partie \ref{partie 3}).
\end{remark}
...
\begin{align}
  \bm{a}\ket{\alpha} &= \alpha\ket{\alpha} = \frac{1}{\sqrt{2}}\left(\frac{\bm{X}}{l}+\frac{il\bm{P}}{\h}\right)\ket{\psi}.
\end{align}
...
\begin{proof}
  La preuve dans le sens direct est le développement débutant au début de \ref{resolution analytique}. Nous ne faisons pas la preuve dans le sens réciproque ($\bm{a}\ket{\psi}=\alpha\ket{\psi}$ pour tout $\alpha\in\mathbb{C}$ implique que $\braket{x}{\psi}=\psi(x)$ est de la forme \ref{fonction d'onde}).
\end{proof}
...
La preuve est une utilisation de la propriété \ref{coherent in fock} que nous venons de démontrer. En effet, nous remarquons que
\begin{align}
...
\end{align}
Le terme $\braket{0}{\alpha}$ a été calculé en \ref{intégrale gaussienne}. Dès lors, tout état cohérent peut donc être généré à partir du vide selon
\begin{align}
...
\subsubsection{Valeur propre de $a$ - résolution algébrique}
