...
Suivons le même raisonnement que celui exploité pour démontrer l'inégalité de Heisenberg dans le cours. On introduit $\mathcal{O}(\lambda) = f^\dagger(\lambda)f(\lambda)$, où $\begin{cases}
  f(\lambda) = \tilde{A}+i\lambda\tilde{B}\\
  \tilde{A} = A-\expval{A}\\
  \tilde{B} = B-\expval{B}
\end{cases}$. Observons que la valeur moyenne de $\mathcal{O}(\lambda)$ est positive. En effet,
\begin{equation*}
  \forall\ket{\psi}\in H, \quad \expval{f^\dagger(\lambda)f(\lambda)}{\psi} = \norm{f(\lambda)\ket{\psi}}^2 \geq 0.
\end{equation*}

Or, $\expval{\mathcal{O}(\lambda)} = \lambda^2\expval{\tilde{B^2}}+\lambda\expval{i[\tilde{A},\tilde{B}]}+\expval{\tilde{A}^2}$. Notons qu'il s'agit d'une équation quadratique en $\lambda$ : ainsi, le minimum doit donc être donné par 
\begin{align}
  \au{\frac{d\expval{\mathcal{O}(\lambda)}}{d\lambda}}{\lambda=\lambda^*} = 0 &= 2\lambda^*\expval{\tilde{B}^2}+\expval{i[\tilde{A},\tilde{B}]}\notag\\
  \lambda^* &= -\frac{\expval{i[\tilde{A},\tilde{B}]}}{2\expval{\tilde{B}^2}} = -\frac{\expval{i[\tilde{A},\tilde{B}]}}{2\left(\Delta \tilde{B}\right)^2}
\end{align}
...
\begin{align}
  \left(\bm{X}-\expval{\bm{X}}\right)\ket{\psi} &= -i\frac{l^2}{\h}\left(\bm{P}-\expval{\bm{P}}\right)
\end{align}
Finalement, en notant que dans la base position l'opérateur impulsion s'écrit $\bm{P}\ket{\psi} = \frac{\h}{i}\frac{\partial}{\partial x}\ket{\psi}$, nous pouvons mettre en évidence l'\emph{équation différentielle}
\begin{align}
