...
\begin{align*}
  \frac{1}{\sqrt{2}}\left(\frac{\bm{X}}{l}+\frac{il\bm{P}}{\h}\right)\ket{\psi} &= \frac{1}{\sqrt{2}}\left(\frac{\expval{X}}{l}+\frac{il\expval{P}}{\h}\right)\ket{\psi}\\
...
\begin{remark}
  Les opérateurs ne sont pas mis en gras dans cette section, car elle fût rédigée avant que je découvre \verb+\bm{}+.
\end{remark}
\label{part 2}
...
\begin{align}
  \bra{\alpha(t)}a^\dagger &= \bra{\alpha(t)}\alpha^*e^{i\omega t}\label{eq:alpha a dagger}
\end{align}
...
\end{align*}
où nous avons exprimé l'égalité sous la \emph{forme normale}. Nous pouvons alors déterminer la moyenne de $P^2$, ce qui donne, sans surprise, un résultat tout à fais similaire à \eqref{Mean X^2}.
\begin{align}
  \label{Mean P^2}
...
\end{align}
\begin{remark}
  Une méthode alternative aurait été de calculer la moyenne de H en exploitant la relation \eqref{Hamiltonien OH normale}. Il aurait alors été suffisant d'invoquer \eqref{eq:a alpha},\eqref{eq:alpha a dagger} et le fait que les états $\ket{\alpha(t)}$ sont normaux pour trouver \eqref{mean H}. 
\end{remark}
\begin{remark}
  Observons directement que, contrairement à la moyenne de la position et de l'impulsion, la moyenne de l'Hamiltonien semble être indépendante du temps. Cela fait sens: l'interprétation de l'Hamiltonien d'un système est celui de l'énergie totale présent dans celui-ci. Or, pour un système isolé - tel notre oscillateur harmonique - l'énergie totale doit être conservé, c'est à dire $\dot{H} = 0$. On aurait pu prédire ce résultat en invoquant le \emph{théorème d'Ehrenfest}. En effet, le théorème nous apprend que pour toute observable A, l'évolution de A peut-être donnée par la relation
  \begin{equation*}
    \frac{d}{dt}\expval{A} = \frac{1}{i\h}\expval{[A,H]}+\expval{\frac{\partial A}{\partial t}}.
  \end{equation*}
  Si on applique ce résultat à l'observable H, le second terme du côté droit se simplifie car H ne dépend pas explicitement du temps. Naturellement, nous avons également que H commute avec lui-même: dès lors, le théorème d'Ehrenfest nous indique que la moyenne de H est \emph{indépendante du temps}, comme nous l'observons.
\end{remark}
...
\begin{equation*}
  \mathcal{L} = \frac{1}{2}mL^2\dot{\theta}^2 + mg\cos\theta 
\end{equation*}
...
Nous voulons appliquer nos résultats à un pendule de masse $m=1\si{kg}$ et de longueur $L=10\si{cm} = 10^{-2}\si{m}$. On supposera que le pendule est en oscillation autour de sa position d'équilibre, avec une amplitude $L\absolutevalue{\theta_{max}} = 1\si{cm} = 10^{-3}\si{m}$.\\

Nous obtenons alors les valeurs suivantes: $\norm{\alpha}^2 \approx 10^{30}$ et $\frac{\Delta H}{\expval{H}} \approx 10^{-16}$ << $1$. L'énergie moyenne des états cohérents constitue alors une excellente approximation de la réalité macroscopique! Cela nous donne la véritable utilité de nos résultats.
...
\begin{align}
  ...
  &= e^{-i\omega t}\ket{\frac{\alpha}{2}e^{-i\omega t}\left(e^{i\lambda t}+e^{-\lambda t}\right),a}\otimes\ket{\frac{\alpha}{2}e^{-i\omega t}\left(e^{-i\lambda t}-e^{i\lambda t}\right),b} = e^{-i\omega t}\ket{\alpha e^{-i\omega t}\cos(\lambda t),a}\otimes\ket{-i\alpha e^{-i\omega t}\sin(\lambda t),b}\label{eq:etat naif evolue}
\end{align}
